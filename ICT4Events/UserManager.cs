...
namespace ICT4Events
{
    class UserManager
    {
        ...
        public User SearchById(int userId)
        {
            ...
            {
                DatabaseConnection con = new DatabaseConnection();
                string Querry = "SELECT ID_USER, ID_EVENTFK, ID_RESERVATIONFK, id_permissionFK, FIRSTNAME, SURNAME, BIRTHDATE, EMAIL, COUNTRY, STREET, HOUSENUMBER, CITY, CELLPHONENUMBER, LOGINNAME, USERNAME, PASSWORDUSER, PROFILEPIC, SUMMARYUSER, PRESENTUSER, RFIDTAG  FROM ICT4_USER WHERE ID_USER = " + userId;
                OracleDataReader reader = con.SelectFromDatabase(Querry);
                User user;
                while (reader.Read())
                {
